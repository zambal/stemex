defmodule Mix.Tasks.Compile.StemexSnowball do
  ...
  def run(_) do
    # force recompilation of stemex
    File.touch!("lib/stemex.ex")

    File.write!(
      "c_src/gen/wrapper.c",
      EEx.eval_file("c_src/wrapper.c.eex", stemmers: Mix.Project.get().stemmers())
    )

    if System.find_executable("snowball") do
      for stemmer <- Mix.Project.get().stemmers() do
        opts = [
          "algorithms/#{stemmer}.sbl",
          "-o",
          "c_src/gen/#{stemmer}",
          "-utf8",
          "-eprefix",
...
defmodule Mix.Tasks.Compile.StemexNif do
  ...
  def run(_) do
    ...
    System.cmd(
      "cc",
      cc_args() ++ ["-o", "priv/Elixir.Stemex_nif.so"] ++ Path.wildcard("c_src/*/*.c"),
  ...
  defp cc_args do
    [i_erts | _] =
      Path.wildcard("#{:code.root_dir()}/erts*/include") |> Enum.sort() |> Enum.reverse()

    [i_ei | _] =
      Path.wildcard("#{:code.root_dir()}/lib/erl_interface*/include")
      |> Enum.sort()
      |> Enum.reverse()

    [l_ei | _] =
      Path.wildcard("#{:code.root_dir()}/lib/erl_interface*/lib")
      |> Enum.sort()
      |> Enum.reverse()
